...
if (isset($_POST["Seni"])) {
    ...
    $_SESSION["Seni"] = true;
    header("Location: buku-pelajaran-admin.php?Seni");
} elseif (isset($_POST["IPA"])) {
    foreach ($mapel as $value) {
        $value = false;
    }
    $_SESSION["IPA"] = true;
    header("Location: buku-pelajaran-admin.php?IPA");
} elseif (isset($_POST["Matematika"])) {
    foreach ($mapel as $value) {
        $value = false;
    }
    $_SESSION["Matematika"] = true;
    header("Location: buku-pelajaran-admin.php?Matematika");
} elseif (isset($_POST["IPS"])) {
    foreach ($mapel as $value) {
        $value = false;
    }
    $_SESSION["IPS"] = true;
    header("Location: buku-pelajaran-admin.php?IPS");
} elseif (isset($_POST["BahasaIndonesia"])) {
    foreach ($mapel as $value) {
        $value = false;
    }
    $_SESSION["BahasaIndonesia"] = true;
    header("Location: buku-pelajaran-admin.php?Bahasa-Indonesia");
    ...
    header("Location: buku-lain-admin.php?Biografi");
} elseif (isset($_POST["CeritaDetektif"])) {
    foreach ($bukulain as $value) {
        $value = false;
    }
    $_SESSION["CeritaDetektif"] = true;
    header("Location: buku-lain-admin.php?CeritaDetektif");
} elseif (isset($_POST["Dongeng"])) {
    foreach ($bukulain as $value) {
        $value = false;
    }
    $_SESSION["Dongeng"] = true;
    header("Location: buku-lain-admin.php?Dongeng");
} elseif (isset($_POST["PengUmum"])) {
    foreach ($bukulain as $value) {
        $value = false;
    }
    $_SESSION["PengUmum"] = true;
    header("Location: buku-lain-admin.php?PengetahuanUmum");
} elseif (isset($_POST["Resep"])) {
    foreach ($bukulain as $value) {
        $value = false;
    }
    $_SESSION["Resep"] = true;
    header("Location: buku-lain-admin.php?Resep");
}
?>
        ...
        <div class="navbar">
            ...
            <nav class="navAtas">
                <ul id="menuList" class="menuList">
                    <li class="koleksi"><a class="aKoleksi" href="halaman-upload.php">Upload</a></li>
                    <li class="koleksi"><a class="aKoleksi" href="tampilan-masukan.php">Saran User</a></li>
